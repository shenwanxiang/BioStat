...
class advance_g_TwowayAnova(ModelBase):

    """
    双因素方差分析法，要求输入一个tsx(Series),dfy(pd.DataFrame),其中tsx必须是定量数据，
            ...
            if not isSeries(tsx) or isCategory(tsx):
                ...
                if len(y_cate_cols) < 2:
                    ...
                    return {'tables': [{'table_json': dfres.reset_index().to_json(orient='index'),
                            ...
                            {'table_json': tb1.reset_index().to_json(orient='index'),
                            'table_html': tb1.to_html(),
                            'table_info': '生成的字段之间的相关系数和p-值表',
                            'chart': ['heatmap', 'line', 'bar']}],
                            'conf': self.get_info(),
                            'msg': msg}, [{'table_df': dfres, 'label': '生成的字段之间的相关系数和p-值表'},
                            {'table_df': tb1, 'label': '生成的字段之间的相关系数和p-值表'}]
